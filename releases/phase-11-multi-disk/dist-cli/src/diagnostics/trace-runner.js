 ...
 * instruction cleanly.
 */
function preStepRecord(machine, cpu, tracer) {
    const ctrl = cpu.intCtrl;
    const nmiPending = ctrl.hasNMI();
    const maskableServicable = ctrl.hasMaskable() && cpu.flags.IF && !cpu.interruptInhibit;
    ...
    if (tracer.enabled('trap') && machine.traps !== null) {
        ...
        if (handler !== undefined) {
            const vector = machine.bios !== null && linear >= 0xF1000 && linear <= 0xF10FF
                ? linear - 0xF1000
                : null;
            tracer.record({
                type: 'trap',
 ...
 */
export function traceRun(machine, opts) {
    ...
    const teardown = opts.instrument !== false
    ...
    switch (e.type) {
        case 'instruction': {
            const bytes = e.bytes
                ...
                .join(' ');
            return (`INSTR ${hex4(e.cs)}:${hex4(e.ip)} (${hex5(e.linear)}) ` +
                `bytes=${bytes} ` +
        ...
        case 'int':
            return (`INT  vec=${hex2(e.vector)} from=${hex4(e.cs)}:${hex4(e.ip)} ` +
                `AX=${hex4(e.ax)} BX=${hex4(e.bx)} CX=${hex4(e.cx)} DX=${hex4(e.dx)}`);
        case 'trap':
            return (`TRAP at=${hex5(e.linear)} ` +
                (e.vector !== null ? `vec=${hex2(e.vector)} ` : '') +
                `AH=${hex2(e.ah)} AL=${hex2(e.al)} ` +
                `BX=${hex4(e.bx)} CX=${hex4(e.cx)} DX=${hex4(e.dx)} ` +
                `DS=${hex4(e.ds)} ES=${hex4(e.es)}`);
        case 'io':
            return (`IO   ${e.dir.toUpperCase()}.${e.size} port=${hex4(e.port)} ` +
                `value=${e.size === 'b' ? hex2(e.value) : hex4(e.value)}`);
        case 'memWrite':
            return (`MEM  W.${e.size} ${hex5(e.addr)} <- ` +
...
function hex5(n) { return n.toString(16).padStart(5, '0'); }
