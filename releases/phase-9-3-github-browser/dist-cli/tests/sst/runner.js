 ...
 */
export function runSSTCase(tc, opts = {}) {
...
function mergeRegs(initial, final) {
 ...
 */
function mergeRam(initial, final) {
    const map = new Map();
    for (const [addr, val] of initial)
        map.set(addr, val & 0xFF);
    for (const [addr, val] of final)
        map.set(addr, val & 0xFF);
    return [...map];
}
function loadState(cpu, s) {
...
function setRegs(cpu, r) {
    ...
    if (r.flags !== undefined)
        cpu.flags.value = r.flags;
}
function collectRegMismatches(cpu, expected, flagsMask, out) {
    const check = (name, exp, act) => {
        if (exp === undefined)
            return;
 ...
 */
function flagsRamLocation(cpu, spDelta) {
    let off;
    if (spDelta === 2)
        off = 0;
    else if (spDelta === 6)
...
function collectRamMismatches(cpu, expected, flagsMask, flagsRamAddr, out) {
    ...
    for (const [addr, expVal] of expected) {
        ...
        let mask = 0xFF;
        if (flagsRamAddr) {
            if (addr === flagsRamAddr.lo)
                mask = flagLoMask;
            else if (addr === flagsRamAddr.hi)
                mask = flagHiMask;
        }
...
export function formatMismatches(result) {
    if (result.pass)
        return `PASS: ${result.case.name}`;
