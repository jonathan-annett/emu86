...
for (let vec = 0; vec < 0x100; vec++) {
    ...
    console.log(`${tag}${hex2(vec)}   |  ${hex4(off)}  |  ${hex4(seg)}   | ${hex5(lin)}`);
}
// ============================================================
...
for (let i = 0; i < events.length; i++) {
    const e = events[i];
    if (!e || e.type !== 'memWrite')
        continue;
    if (e.addr >= 0x400)
        continue;
    const vec = (e.addr >> 2) & 0xFF;
    if (!interesting.has(vec))
        continue;
    const which = (e.addr & 2) === 0 ? 'OFF' : 'SEG';
    const sz = e.size === 'w' ? 'W' : 'B';
    const tag = (svcEventIdx >= 0 && i > svcEventIdx) ? 'POST-SVC' : 'pre-svc ';
    console.log(`  [${tag} idx=${i}] vec=${hex2(vec)} ${which} ${sz} addr=${hex5(e.addr)} val=${e.size === 'b' ? hex2(e.value) : hex4(e.value)}`);
}
// ============================================================
...
console.log('## Section 5a: IDT state reconstructed AT intService idx');
{
    ...
    for (let i = 0; i < svcEventIdx; i++) {
        ...
        if (e.size === 'b') {
            ...
            idtAtSvc[e.addr] = e.value & 0xFF;
            if (e.addr + 1 < 0x400)
                idtAtSvc[e.addr + 1] = (e.value >> 8) & 0xFF;
        }
    }
...
function dumpRange(label, baseLin, len = 32) {
    console.log(`  ${label}:`);
...
for (let row = 0; row < 12; row++) {
    ...
    const cells = [];
    for (let col = 0; col < 16; col++)
        cells.push(hex2(m.memory.readByte(lin + col)));
