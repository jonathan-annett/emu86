...
import { installCGAMirror, OneShotPrefixSink, CLEAR_AND_HOME, } from '../../src/diagnostics/cga-mirror.js';
const DEFAULT_IMAGE = 'reference/elks-images/fd1440-minix.img';
import { loadDiskFromPath, parseSecondaryFlags, } from './secondary-disk.js';
function loadDisk(imagePath) {
...
function printBanner(image, secondary) {
...
async function main() {
    ...
    const secondary = parsed.secondary
        ...
        : null;
    const secondaryClass = parsed.secondary?.diskClass ?? undefined;
    ...
    const pumpInterval = setInterval(() => {
        while (console_.hasInput()) {
            ...
            if (scans.length > 0)
                machine.keyboardController.injectScancodes(scans);
        }
    }, 10);
