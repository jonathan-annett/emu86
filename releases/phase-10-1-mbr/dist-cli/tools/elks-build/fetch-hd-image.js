 ...
 * `hd32mbr-minix.img`, used by the MBR-boot integration tests.
 *
 * Usage:
...
async function fetchImage(spec) {
    ...
    if (buf.length !== spec.bytes) {
        throw new Error(`fetched size for ${spec.name} (${buf.length}) does not match expected ${spec.bytes}; aborting`);
    }
    writeFileSync(fullPath, buf);
    console.log(`Wrote ${buf.length} bytes to ${fullPath}.`);
}
function selectImages(args) {
    if (args.length === 0) {
