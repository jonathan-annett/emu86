...
function pressRelease(scan) {
    return [scan & 0x7F, (scan & 0x7F) | 0x80];
}
function withShift(scan) {
    return [
        SCAN_LSHIFT,
        scan & 0x7F,
        (scan & 0x7F) | 0x80,
 ...
 */
export function translateByte(byte) {
    const b = byte & 0xFF;
    ...
    if (b === 0x08 || b === 0x7F)
        return pressRelease(SCAN_BACKSPACE);
    if (b === 0x09)
        return pressRelease(SCAN_TAB);
    if (b === 0x0A || b === 0x0D)
        return pressRelease(SCAN_ENTER);
    if (b === 0x1B)
        return pressRelease(SCAN_ESC);
    if (b === 0x20)
        return pressRelease(SCAN_SPACE);
    ...
    if (b < 0x20 || b > 0x7E)
        return []; // unprintable / non-ASCII
    const ch = String.fromCharCode(b);
    const direct = UNSHIFTED[ch];
    if (direct !== undefined)
        return pressRelease(direct);
    const shifted = SHIFTED[ch];
    if (shifted !== undefined)
        return withShift(shifted);
    return [];
}
/**
