...
/** Drive number assigned to the currently-attached disk by class. */
...
export function int13Handler(cpu, ctx) {
    ...
    switch (ah) {
        ...
        case 0x04: {
            ...
            // The single attached disk is registered as either 0x00 (floppy) or
            // 0x80 (hard-disk) per BiosContext.diskClass. A request for the wrong
            // drive number bounces with BAD_COMMAND — multi-disk support (one of
            // each simultaneously) is a future brief.
            const expectedDl = bootDriveNumber(ctx.diskClass);
            if (dl !== expectedDl) {
        ...
        case 0x08: {
            ...
            //   DL = number of drives of this class attached (1 in our v0)
            ...
            const expectedDl = bootDriveNumber(ctx.diskClass);
            if (dl !== expectedDl) {
                // Caller asked about a drive we don't have (e.g., DL=0x80 with a
                // floppy mounted, or DL=0x00 with an HD mounted). Standard error.
                setDiskStatus(cpu, bda, DISK_STATUS_BAD_COMMAND);
                cpu.regs.BL = 0;
                cpu.regs.CH = 0;
                cpu.regs.CL = 0;
            ...
            cpu.regs.DH = maxHead;
            cpu.regs.DL = 1; // one drive of this class
            cpu.regs.BL = ctx.diskClass === 'hard-disk' ? 0x00 : 0x04;
            if (ctx.diskClass === 'hard-disk') {
