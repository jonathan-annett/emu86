...
function inferStandardGeometry(byteSize) {
    for (const g of STANDARD_GEOMETRIES) {
        if (totalSectors(g) * SECTOR_SIZE === byteSize)
            return g;
    }
    return null;
}
export class InMemoryDisk {
    ...
    constructor(opts) {
        ...
        if (opts.contents) {
            if (opts.contents.length > bytes) {
                throw new Error(`InMemoryDisk: initial contents (${opts.contents.length} B) exceed disk size (${bytes} B)`);
            }
    ...
    writeSector(lba, data) {
        ...
        if (data.length !== SECTOR_SIZE) {
            throw new Error(`InMemoryDisk: writeSector data must be ${SECTOR_SIZE} bytes (got ${data.length})`);
        }
        this.#data.set(data, lba * SECTOR_SIZE);
    }
    #checkLba(lba) {
        if (!Number.isInteger(lba) || lba < 0 || lba >= this.sectorCount) {
            throw new Error(`InMemoryDisk: LBA ${lba} out of range (sectorCount=${this.sectorCount})`);
        }
    }
}
/**
...
export class NodeFileDisk {
    ...
    constructor(opts) {
        ...
        if (!geometry) {
            ...
            if (!inferred) {
                closeSync(this.#fd);
                this.#fd = null;
                throw new Error(`NodeFileDisk: file size ${bytes} bytes does not match any standard floppy geometry; pass an explicit geometry`);
            }
            geometry = inferred;
        }
        else {
            ...
            if (expected !== bytes) {
                closeSync(this.#fd);
                this.#fd = null;
                throw new Error(`NodeFileDisk: explicit geometry expects ${expected} bytes, file has ${bytes} bytes`);
            }
        }
        ...
        while (offset < bytes) {
            ...
            if (n === 0)
                break; // EOF reached early; trust whatever we got
            offset += n;
        }
        if (offset !== bytes) {
            closeSync(this.#fd);
            this.#fd = null;
            throw new Error(`NodeFileDisk: short read on initial slurp (got ${offset} of ${bytes})`);
        }
    }
    /**
    ...
    writeSector(lba, data) {
        ...
        this.#checkLba(lba);
        if (data.length !== SECTOR_SIZE) {
            throw new Error(`NodeFileDisk: writeSector data must be ${SECTOR_SIZE} bytes (got ${data.length})`);
        }
        ...
        while (written < SECTOR_SIZE) {
            const n = writeSync(this.#fd, this.#cache, offset + written, SECTOR_SIZE - written, offset + written);
            if (n === 0)
                break;
            written += n;
        }
    ...
    #checkLba(lba) {
        if (!Number.isInteger(lba) || lba < 0 || lba >= this.sectorCount) {
            throw new Error(`NodeFileDisk: LBA ${lba} out of range (sectorCount=${this.sectorCount})`);
        }
    }
}
