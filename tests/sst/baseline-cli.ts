...
import type { SSTResult } from './runner.js';
...
function parseArgs(argv: string[]): CliOpts {
  const opts: CliOpts = { bail: false, masks: new Map() };
  for (const a of argv) {
    if (a.startsWith('--filter=')) opts.filter = new RegExp(a.slice('--filter='.length), 'i');
    else if (a === '--bail') opts.bail = true;
    else if (a.startsWith('--mask=')) {
      const [id, hex] = a.slice('--mask='.length).split('=');
      if (!id || !hex) throw new Error(`bad --mask: ${a}`);
...
function main(): number {
  ...
  for (const path of files) {
    ...
    console.log(line);
    if (dirty) {
      for (const f of failures) console.log('  ' + formatMismatches(f).replace(/\n/g, '\n  '));
      for (const e of errors) console.log(`  THREW: ${e.name}: ${e.msg}`);
    }
