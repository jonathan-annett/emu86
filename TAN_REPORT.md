...
**Outcome:** ✅ **Gold delivered — and human-verified in real browser tabs** (Jonathan, 2026-07-14): telnet across tabs works, and **FTP works too — a binary was transferred between tabs and executed**: `/bin/tetris` copied off one tab's machine via ftp and played from the other tab's home directory. Byte-exact binary transfer over ktcp↔ktcp TCP (FTP's second data connection and all) across a LAN made of BroadcastChannel messages, then an interactive ANSI game over the serial console. No test in this repo proves as much per second as that sentence. Two full WorkerHost machines — exactly what two browser tabs run — boot the stock image on one shared Tab Area Network, lease themselves unique identities, join with the guest's own `net start ne0`, and then **machine A telnets to machine B and receives B's real login prompt**, served by B's unmodified telnetd over real ktcp-to-ktcp TCP. No host-side TCP engine exists anywhere; only ethernet frames moved. En route, the milestone surfaced a genuine distributed-systems bug (ktcp's connect clock vs. cross-tab ARP latency) whose fix — lease-directory proxy-ARP — makes the TAN robust for real browser tabs, not just tests.
