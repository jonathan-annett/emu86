 ...
 */
export function findBootopts(image) {
 ...
 */
export function hasSerialConsole(image) {
    const offset = findBootopts(image);
    if (offset === null)
        return false;
    return readBlockLines(image, offset).some((line) => line.trim().startsWith('console=ttyS0'));
}
/**
...
export function patchBootoptsForSerial(image, extraLines = []) {
    ...
    if (text.length > BOOTOPTS_SIZE - 1) {
        throw new Error(`patchBootoptsForSerial: rewritten /bootopts is ${text.length} bytes ` +
 ...
 */
function readBlockLines(image, offset) {
    let text = '';
