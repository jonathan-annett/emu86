...
import { AUTHENTIC_CYCLES_PER_MS, RealTimePacer, } from './pacing.js';
const SIZE_TABLE = [
    // ---- Floppies ----
...
export class WorkerHost {
    ...
    async #runPacedLoop() {
        ...
        while (!this.#stopping && this.#machine !== null && this.#console !== null) {
            ...
            const budget = Math.min(this.#adaptiveBatch, this.#pacer.instructionBudget(cycles));
     ...
     */
    #stepInstructions(machine, budget) {
        const cpu = machine.cpu;
        let executed = 0;
        try {
    ...
    async #boot(config) {
        ...
        if (config.cpuSpeed !== undefined)
            this.#pacer.setMode(config.cpuSpeed);
    ...
    async #resolveSlot(slotName, spec, extraBootoptsLines = []) {
        ...
        if (spec.imageBytes) {
            ...
            if (!this.#fetchImage) {
                throw new Error(`WorkerHost: ${slotName} disk imageUrl supplied but no fetchImage callback configured`);
            }
            bytes = await this.#fetchImage(spec.imageUrl);
        }
        else {
            throw new Error(`WorkerHost: ${slotName} disk spec must carry imageBytes or imageUrl`);
        }
        let geometry = spec.geometry;
        let diskClass = spec.diskClass;
        if (!geometry) {
            const inferred = inferFromSize(bytes.length);
            if (!inferred) {
                throw new Error(`WorkerHost: ${slotName} disk: unrecognised image size ${bytes.length} bytes. ` +
...
const yieldMacrotask = (() => {
    const isRealWorker = typeof globalThis.importScripts === 'function';
