 ...
 */
export const NE2K_DEFAULT_MAC = [0x02, 0x65, 0x6d, 0x75, 0x38, 0x36];
...
export class NE2000 {
    ...
    #rsar = 0;
    #rbcr = 0;
    #par = new Uint8Array(6);
    #mar = new Uint8Array(8);
    // ---- remote DMA engine ----
    #remoteMode = 'idle';
    #dmaAddr = 0;
    #dmaRemaining = 0;
    ...
    injectFrame(frame) {
        ...
        if (next >= this.#pstop)
            next = this.#pstart + (next - this.#pstop);
        ...
        const writeRing = (offsetInPacket, value) => {
    ...
    writeByte(port, value) {
        ...
        if (offset === REG_RESET)
            return; // write half of the reset pulse — no-op
        if (offset === REG_CR) {
            this.#writeCR(v);
            return;
        }
        const page = (this.#cr >> 6) & 0x03;
        if (page === 1)
            this.#writePage1(offset, v);
        else
            this.#writePage0(offset, v);
    }
    /**
    ...
    #writePage0(offset, v) {
        switch (offset) {
            ...
            case REG_TPSR_TSR:
                ...
                return;
            case REG_TBCR0:
                this.#tbcr = (this.#tbcr & 0xff00) | v;
                return;
            case REG_TBCR1:
                this.#tbcr = (this.#tbcr & 0x00ff) | (v << 8);
                return;
            case REG_ISR:
                ...
                return;
            case REG_RSAR0:
                this.#rsar = (this.#rsar & 0xff00) | v;
                return;
            case REG_RSAR1:
                this.#rsar = (this.#rsar & 0x00ff) | (v << 8);
                return;
            case REG_RBCR0:
                this.#rbcr = (this.#rbcr & 0xff00) | v;
                return;
            case REG_RBCR1:
                this.#rbcr = (this.#rbcr & 0x00ff) | (v << 8);
                return;
            case REG_RSR_RCR:
                this.#rcr = v;
                return;
    ...
    #readPage1(offset) {
        if (offset >= 0x01 && offset <= 0x06)
            return this.#par[offset - 1] ?? 0;
        if (offset === REG_ISR)
            return this.#curr; // page 1 +0x07 = CURR
        if (offset >= 0x08 && offset <= 0x0f)
            return this.#mar[offset - 8] ?? 0;
        if (offset === REG_CR)
            return this.#crRead();
        return 0xff;
    }
