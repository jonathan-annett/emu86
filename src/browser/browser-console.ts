...
export interface BrowserConsoleOptions {
  /**
   * Sink for outbound bytes (writeChar). Worker host wires this to a
   * coalescing TX buffer that gets flushed as `tx` messages.
   */
  txSink: (byte: number) => void;
}

export class BrowserConsole implements Console {
  readonly #txSink: (byte: number) => void;
   ...
   * encoder.
   */
  injectInput(bytes: Uint8Array | readonly number[]): void {
    for (const b of bytes) this.#inputQueue.push(b & 0xFF);
  }
