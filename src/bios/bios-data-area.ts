...
export class BiosDataArea {
  ...
  writeDword(offset: number, value: number): void {
  ...
  isKbBufferFull(): boolean {
    ...
    let nextTail = tail + 2;
    if (nextTail >= end) nextTail = start;
   ...
   */
  pushKey(asciiCode: number, scancode: number): boolean {
    if (this.isKbBufferFull()) return false;
    const tail = this.readWord(BDA.KBBUF_TAIL);
    const start = this.readWord(BDA.KBBUF_START_PTR);
    const end = this.readWord(BDA.KBBUF_END_PTR);
   ...
   */
  peekKey(): number {
    if (this.isKbBufferEmpty()) return -1;
    const head = this.readWord(BDA.KBBUF_HEAD);
  ...
  popKey(): number {
    ...
    const end = this.readWord(BDA.KBBUF_END_PTR);
    let nextHead = head + 2;
    if (nextHead >= end) nextHead = start;
