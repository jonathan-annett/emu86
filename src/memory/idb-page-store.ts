...
export interface IndexedDBPageStoreOptions {
  /** Database name. Default `emu86-pages`. */
  databaseName?: string;
}
...
export class IndexedDBPageStore implements PageStore {
  ...
  constructor(opts: IndexedDBPageStoreOptions = {}) {
   ...
   * is safe in practice.
   */
  async *loadAll(): AsyncIterable<readonly [pageId: number, data: Uint8Array]> {
    ...
    let resolveStep!: (cursor: IDBCursorWithValue | null) => void;
    let rejectStep!: (err: unknown) => void;
    let pending = new Promise<IDBCursorWithValue | null>((res, rej) => {
      resolveStep = res;
      rejectStep = rej;
    });
    ...
    request.onsuccess = () => {
      ...
      pending = new Promise<IDBCursorWithValue | null>((res, rej) => {
        resolveStep = res;
        rejectStep = rej;
      });
    ...
    while (true) {
      ...
      if (typeof key !== 'number') {
        throw new Error(
          `IndexedDBPageStore: expected number key, got ${typeof key}`
        );
      }
      const value: unknown = cursor.value;
      if (!(value instanceof Uint8Array)) {
        throw new Error(
          `IndexedDBPageStore: expected Uint8Array value at key ${key}`
        );
      }
  ...
  #awaitTransaction(tx: IDBTransaction): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      tx.onerror = () =>
        reject(tx.error ?? new Error('IndexedDBPageStore: transaction error'));
      tx.onabort = () =>
        reject(tx.error ?? new Error('IndexedDBPageStore: transaction aborted'));
    });
  }
}
