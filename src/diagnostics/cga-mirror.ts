...
export interface CGAMirrorSink {
  /**
   * Invoked once per byte the mirror emits. The byte may be a printable
   * ASCII character, a control character (0x07 BEL, 0x0A LF), a high-bit
   * code-page-437 glyph, or part of an ANSI cursor-positioning sequence
   * (ESC, '[', digits, ';', 'H') the mirror writes around character
   * payloads. The sink does not need to demultiplex these — any
   * terminal-compatible target (host stdout) interprets the stream
   * correctly as a whole.
   */
  writeChar(byte: number): void;
}

export interface InstallCGAMirrorOptions {
  sink: CGAMirrorSink;
  /**
   * Override the watched range. Default: 0xB8000-0xBFFFF (the entire
   * CGA text-mode region including alternate pages). Tests sometimes
   * narrow the range to a single 80x25 page.
   */
  start?: number;
  end?: number;
}
 ...
 */
export function installCGAMirror(
  machine: IBMPCMachine,
  opts: InstallCGAMirrorOptions,
): () => void {
  const { sink } = opts;
  ...
  mem.writeByte = (addr: number, v: number) => {
    ...
    if (addr < start || addr >= end || (addr & 1) !== 0) return;
...
function emitCursorPosition(sink: CGAMirrorSink, row: number, col: number): void {
...
function emitDecimal(sink: CGAMirrorSink, n: number): void {
  const s = String(n);
  for (let i = 0; i < s.length; i++) sink.writeChar(s.charCodeAt(i));
}
 ...
 */
export class CapturingCGASink implements CGAMirrorSink {
  ...
  get text(): string {
    let s = '';
    for (const c of this.#bytes) s += String.fromCharCode(c);
    return s;
  }
 ...
 */
export class OneShotPrefixSink implements CGAMirrorSink {
  readonly #inner: CGAMirrorSink;
  readonly #prefix: readonly number[];
  #fired = false;

  constructor(inner: CGAMirrorSink, prefix: readonly number[]) {
    this.#inner = inner;
    this.#prefix = prefix;
  }
  ...
  writeChar(byte: number): void {
    if (!this.#fired) {
      this.#fired = true;
      for (const b of this.#prefix) this.#inner.writeChar(b);
    }
 ...
 */
export const CLEAR_AND_HOME: readonly number[] =
