...
OPCODE_TABLE[0x88] = (cpu) => {                    // MOV r/m8, r8
...
OPCODE_TABLE[0x89] = (cpu) => {                    // MOV r/m16, r16
  const m = decodeModRM(cpu);
  const dst = rmOperand(cpu, m);
  const src = regOperand(cpu, m.reg);
...
OPCODE_TABLE[0x8A] = (cpu) => {                    // MOV r8, r/m8
  const m = decodeModRM(cpu);
...
OPCODE_TABLE[0x8B] = (cpu) => {                    // MOV r16, r/m16
  const m = decodeModRM(cpu);
  const dst = regOperand(cpu, m.reg);
  const src = rmOperand(cpu, m);
  dst.write16(src.read16());
...
OPCODE_TABLE[0x8C] = (cpu) => {                    // MOV r/m16, sreg
  const m = decodeModRM(cpu);
  const dst = rmOperand(cpu, m);
  dst.write16(cpu.regs.getSreg(m.reg & 3));
};

OPCODE_TABLE[0x8E] = (cpu) => {                    // MOV sreg, r/m16
  const m = decodeModRM(cpu);
  const src = rmOperand(cpu, m);
...
OPCODE_TABLE[0xA2] = (cpu) => {                    // MOV [moffs8], AL
  const offset = cpu.fetchWord();
  cpu.memory.writeByte(linearAddress(cpu.dataSegment(), offset), cpu.regs.AL);
};

OPCODE_TABLE[0xA3] = (cpu) => {                    // MOV [moffs16], AX
  const offset = cpu.fetchWord();
  const seg = cpu.dataSegment();
  const v = cpu.regs.AX;
...
OPCODE_TABLE[0xC6] = (cpu) => {                    // MOV r/m8, imm8
  const m = decodeModRM(cpu);
  const imm = cpu.fetchByte();
  rmOperand(cpu, m).write8(imm);
};

OPCODE_TABLE[0xC7] = (cpu) => {                    // MOV r/m16, imm16
  const m = decodeModRM(cpu);
