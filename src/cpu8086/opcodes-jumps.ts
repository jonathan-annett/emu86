...
function makeJcc(predicate: (cpu: CPU8086) => boolean): OpcodeHandler {
  return (cpu) => {
...
OPCODE_TABLE[0xE0] = (cpu) => {                    // LOOPNZ / LOOPNE
  const disp = signedByte(cpu.fetchByte());
...
OPCODE_TABLE[0xE1] = (cpu) => {                    // LOOPZ / LOOPE
  const disp = signedByte(cpu.fetchByte());
  cpu.regs.CX = (cpu.regs.CX - 1) & 0xFFFF;
  if (cpu.regs.CX !== 0 && cpu.flags.ZF) {
    cpu.regs.IP = (cpu.regs.IP + disp) & 0xFFFF;
...
OPCODE_TABLE[0xE3] = (cpu) => {                    // JCXZ — no decrement
  const disp = signedByte(cpu.fetchByte());
  if (cpu.regs.CX === 0) {
    cpu.regs.IP = (cpu.regs.IP + disp) & 0xFFFF;
  }
