...
/**
 * Opcode handler signature. Takes the CPU, mutates its state, done.
 *
 * The handler's contract:
 *   - On entry, IP has been advanced past the opcode byte itself.
 *   - Handler fetches any remaining bytes (ModR/M, displacement, immediate)
 *     via `cpu.fetchByte()` / `cpu.fetchWord()`, which advance IP.
 *   - Handler performs the effect (register/flag/memory writes, IP jumps).
 *   - No return value. If the handler throws, it propagates out of step().
 *
 * Adding a new instruction: pick its opcode byte, write a handler with this
 * signature, assign OPCODE_TABLE[0xXX] = handler. If it shares decoding
 * structure with other instructions (e.g., all ALU ops with ModR/M), factor
 * a shared decode helper before writing each variant.
 */
export type OpcodeHandler = (cpu: CPU8086) => void;

/**
 * 256-slot dispatch table. Entries are `undefined` for unimplemented opcodes,
 * at which point the CPU throws InvalidOpcodeError.
 */
export const OPCODE_TABLE: (OpcodeHandler | undefined)[] = new Array(256);
...
// ============================================================
function makeSegmentPrefix(sregIndex: 0 | 1 | 2 | 3): OpcodeHandler {
  return (cpu) => {
