...
import { flagsDec16, flagsDec8, flagsInc16, flagsInc8, flagsLogic16, flagsLogic8, flagsSub16, flagsSub8 } from './flag-helpers.js';
...
OPCODE_TABLE[0xF6] = (cpu) => {
  ...
  switch (m.reg) {
    ...
    case 4: mul8(cpu, op.read8()); return;             // MUL r/m8
    case 5: imul8(cpu, op.read8()); return;            // IMUL r/m8
    case 6: div8(cpu, op.read8()); return;             // DIV r/m8
    case 7: idiv8(cpu, op.read8()); return;            // IDIV r/m8
  }
};
...
OPCODE_TABLE[0xF7] = (cpu) => {
  ...
  switch (m.reg) {
    ...
    case 4: mul16(cpu, op.read16()); return;
    case 5: imul16(cpu, op.read16()); return;
    case 6: div16(cpu, op.read16()); return;
    case 7: idiv16(cpu, op.read16()); return;
  }
};
...
function mul8(cpu: CPU8086, x: number): void {
...
function mul16(cpu: CPU8086, x: number): void {
...
function imul8(cpu: CPU8086, x: number): void {
...
function imul16(cpu: CPU8086, x: number): void {
...
function div8(cpu: CPU8086, divisor: number): void {
  if (divisor === 0) { serviceInterrupt(cpu, 0); return; }
  const dividend = cpu.regs.AX;
  const q = Math.trunc(dividend / divisor);
  const r = dividend - q * divisor;
  if (q > 0xFF) { serviceInterrupt(cpu, 0); return; }
  cpu.regs.AL = q & 0xFF;
  cpu.regs.AH = r & 0xFF;
}

function div16(cpu: CPU8086, divisor: number): void {
  if (divisor === 0) { serviceInterrupt(cpu, 0); return; }
  // 32-bit dividend = DX:AX, fits in JS Number safely (max 2^32 - 1).
  const dividend = cpu.regs.DX * 0x10000 + cpu.regs.AX;
  const q = Math.trunc(dividend / divisor);
  const r = dividend - q * divisor;
  if (q > 0xFFFF) { serviceInterrupt(cpu, 0); return; }
  cpu.regs.AX = q & 0xFFFF;
  cpu.regs.DX = r & 0xFFFF;
}

function idiv8(cpu: CPU8086, divisor: number): void {
  if (divisor === 0) { serviceInterrupt(cpu, 0); return; }
  ...
  if (q < -0x7F || q > 0x7F) { serviceInterrupt(cpu, 0); return; }
  cpu.regs.AL = q & 0xFF;
  cpu.regs.AH = r & 0xFF;
}

function idiv16(cpu: CPU8086, divisor: number): void {
  if (divisor === 0) { serviceInterrupt(cpu, 0); return; }
  ...
  let r = dividend - q * sd;
  if (cpu.repPrefix !== null) q = -q;
  if (q < -0x7FFF || q > 0x7FFF) { serviceInterrupt(cpu, 0); return; }
  cpu.regs.AX = q & 0xFFFF;
  cpu.regs.DX = r & 0xFFFF;
}
