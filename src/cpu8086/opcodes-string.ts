 ...
 */
function repLoop(cpu: CPU8086, zfBreaksOn: boolean, prefix: 'F2' | 'F3'): void {
  ...
  if (IS_STRING_OP[next] !== 1) {
    ...
    try { handler(cpu); } finally { cpu.repPrefix = null; }
    return;
  }
...
function readWord(cpu: CPU8086, segment: number, offset: number): number {
