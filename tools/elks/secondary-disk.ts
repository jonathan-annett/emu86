 ...
 */
export function loadDiskFromPath(path: string): InMemoryDisk {
  return loadDiskFromPathWithClass(path).disk;
}

export function loadDiskFromPathWithClass(
  path: string,
  classOverride?: DiskClass,
): LoadedDisk {
  const bytes = new Uint8Array(readFileSync(path));
  const inferred = inferFromSize(bytes.length);
  if (!inferred) {
    throw new Error(
      `Unrecognised image size ${bytes.length} bytes for ${path}. ` +
...
export interface ParsedSecondary {
  /** Path passed via --hdb or --fd1. Always set when this object exists; the
   *  enclosing `ParsedArgs.secondary` field is null when neither flag was
   *  given. */
  path: string;
  /**
   * Class derived from which flag was used. `--hdb` pins hard-disk;
   * `--fd1` pins floppy. `--secondary-class` overrides either.
   */
  diskClass: DiskClass | null;
}

export interface ParsedArgs {
  /** Positional arguments left after stripping our flags. Original order preserved. */
  positional: string[];
  /** Secondary slot, or null if no `--hdb` / `--fd1` was passed. */
  secondary: ParsedSecondary | null;
}
 ...
 */
export function parseSecondaryFlags(argv: readonly string[]): ParsedArgs {
  const positional: string[] = [];
  let path: string | null = null;
  let diskClass: DiskClass | null = null;
  let classOverride: DiskClass | null = null;
  let i = 0;
  while (i < argv.length) {
    const a = argv[i];
    if (a === '--hdb' || a === '--fd1') {
    ...
    if (a === '--secondary-class') {
      ...
      throw new Error(
        `--secondary-class requires "floppy" or "hard-disk" (got ${next ?? '(none)'}).`,
      );
    }
    positional.push(a as string);
    i++;
  }
  if (path === null) return { positional, secondary: null };
  return {
    positional,
    secondary: {
      path,
